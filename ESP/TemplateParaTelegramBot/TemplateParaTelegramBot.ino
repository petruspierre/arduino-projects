/*
 * 
 *                                     TEMPLATE PARA BOT DO TELEGRAM
 *                                      Usar como base de projetos
 *                                       Criado por Petrus Pierre
 *  
 *  Para usar: primeiramente altere a rede WiFi, token do seu bot, inicie o programa para ver seu ID 
 *  pela Serial ao mandar mensagem para o bot. E então configure os comandos que desejar, para isso, 
 *  basta seguir os exemplos com as funções existentes e criar sua própria função, adicionando ela 
 *  na função quandoNovaMensagem
 * 
 */
...
#define SENHA "SenhaDaRede" 

 // Declaração das variáveis: 
#define BOT_TOKEN "000000000:XXXXXXXXXXXXXXXXXXXXXXXXXXXXXX" // Seu token gerado pelo bot vai aqui
...
UniversalTelegramBot bot(BOT_TOKEN, client); // Instanciando nosso bot
 ...
 String idsValidos[QUANTIDADE_IDS] = {"000000000"}; // Coloque nesse vetor os usuarios que vao ser permitidos
...
void quandoNovaMensagem(int novasMensagens){
  ...
  for(int i=0;i<novasMensagens; i++){ // Executa a quantidade de vezes que recebeu mensagem dentro do intervalo
    ...
    // OBS: o equalsIgnoreCase significa que não importa se é maiusculo ou minusculo
...
void quandoInicia(String idChat, String nomeUsuario){
  ...
  messagem += listaDeComandos(); // Chama a função que retorna a lista de comandos
  bot.sendMessage(idChat, mensagem, "HTML");
}

String listaDeComandos(){
  String mensagem = "Os comandos disponíveis são:\n\n";
  mensagem += "<b>\"" + INICIO + "\"</b>: Para iniciar o bot";
